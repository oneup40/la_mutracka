            ...
            <div className='task-choice-field-grid'>
                {Universe.regions.withTag('grail-tablet').map(region => {
                    ...
                    return (
                        ...
                        // <div key={region.key} className={['task-choice', region.field.key, region.key].join(' ')} onClick={() => onClick(region)}>{region.fullName()}</div>;
                })}
            </div>
        </Popup>
    );
...
export function StartWeaponTask({id, onSubmit}) {
    ...
    return (
        <Popup
            ...
            arrow={false}
        >
            <div className='task-choice-weapon-grid'>
                {Universe.items.withTag('start-weapon').map(item => {
                    // return <div key={item.key} className={['task-choice','weapon',item.key].join(' ')} onClick={() => onClick(item)}>{item.name}</div>;
                    return (<img
                                key={item.key}
...
export function TransitionTask({id, connection, connectionMap, onSubmit}) {
    ...
    if (connectionMap.has(connection.key)) {
        ...
        return (
            <Popup
                className='task-root'
                trigger={<div className='task-root'>Transition: {srcConn.name}</div>}
                position="right top"
                arrow={false}
            >
                <div className='task-choice-connection-grid'>
                    {srcConn.candidates().filter(dstConn => dstConn !== srcConn && (srcConn.tags.has('alias') || !connectionMap.has(dstConn.key))).map(dstConn => {
                        let icon = null;
                        if (dstConn.region.field.icon !== null) {
                            let classes = ["region-field-icon"];
                            console.log(dstConn.region.isBackside());
                            if (dstConn.region.isBackside()) {
                                classes.push("region-backside");
                            }
                            icon = <img className={classes.join(" ")} alt="" src={`/assets/img/${dstConn.region.field.icon}.png`} />;
                        }

                        return (
                            <div
                                key={dstConn.key}
                                className={['task-choice','connection',dstConn.region.field.key,dstConn.region.key].join(' ')}
                                onClick={() => onClick(dstConn)}
                            >
                                {icon}
                                <span className='region-field-name'>{dstConn.region.field.name}</span>
                                <span className='region-name'>{dstConn.region.name}</span>
                            </div>
                        );
                    })}
                </div>
